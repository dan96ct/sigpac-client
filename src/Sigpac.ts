 ...
 */
const localizacion = async (capa: string, coords: {lng: number, lat: number}, configUser: ConfigUser): Promise<Geojson> => {
 ...
 */
const buscar = async (data: { [key: string]: number }, configUser: ConfigUser): Promise<Geojson> => {
 ...
 */
const consulta = async (capa: string, data: { [key: string]: any }, configUser: ConfigUser): Promise<Consulta> => {
     const configOverride = {...config, ...configUser};
     enum capas {
      parcela = 'parcela',
      recinto = 'recinto',
