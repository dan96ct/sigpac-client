import { Feature, Geojson } from "../types.js";
 ...
 * @returns {Geojson} Objeto con las coordenadas de las parcelas cambiadas
 */
  ...
  const result = {
    ...
    features: geojson.features.map((feature: Feature) => {
      return {
        ...
        geometry: {
          ...feature.geometry,
          coordinates: feature.geometry.coordinates.map((subpolygon) =>
            subpolygon.map((pair) => {
              return proj4(
                firstProjection,
                projection,
